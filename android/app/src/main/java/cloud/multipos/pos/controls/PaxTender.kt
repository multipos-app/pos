...
class PaxTender (): Tender (Jar ()) {
	 ...
	 override fun dialogView (): DialogView {
	 ...
	 override fun payment (): Tender {
		  ...
		  var authString = (total).toInt ().toString ()
	 ...
	 inner class AuthHandler (val tender: PaxTender): Handler (Looper.getMainLooper ()) {
		  ...
		  override fun handleMessage (m: Message) {
				...
				Logger.i ("auth handler... ${m.what} ${result.stringify ()}")
				...
				when (m.what) {
					 ...
					 PosConst.PAYMENT_COMPLETE -> {
						  ...
						  var tt = TicketTender (Jar ()
									 						  ...
									 						  .put ("sub_tender_type", result.getString ("card_brand").lowercase ())
