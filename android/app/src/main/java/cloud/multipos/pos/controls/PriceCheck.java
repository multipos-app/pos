...
public class PriceCheck extends Control {

	 public PriceCheck () { super (); }

	 public void controlAction (Jar Jar) {
		  
		  String input = Pos.app.input.getString ();
		  ...
		  if (input.length () > 0) {
				...
				DbResult itemResult  = new DbResult (select, Pos.app.db);
				if (itemResult.fetchRow ()) {
					 
					 Jar item = itemResult.row ();
					 if (item.getDouble ("price") != 0) {

						  PosDisplays.message (item.getString ("sku") + " " +
													  item.getString ("item_desc") + " " +
													  Strings.currency (item.getDouble ("price"), false));
					 }
				}
				else {
